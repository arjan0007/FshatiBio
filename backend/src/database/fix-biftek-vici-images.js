...
// URL të vlefshme për foto reale të biftekut nga Pexels dhe Pixabay
const biftekImageUrls = [
...
async function fixBiftekViciImages() {
  try {
    console.log('🥩 Fixing Biftek viçi images...');
    
    const products = await pool.query(`
      SELECT p.id, p.name, p.image_urls
      FROM products p
      WHERE p.name = 'Biftek viçi'
    ...
    if (products.rows.length === 0) {
      console.log('⚠️  Product "Biftek viçi" not found');
      process.exit(0);
    }
    ...
    const numImages = 3; // 3 foto për biftek
    const downloadedUrls = [];
    const allUrls = [...biftekImageUrls, ...alternativeUrls];
    
    console.log(`📸 Downloading ${numImages} images for: ${product.name}`);
    
    for (let i = 0; i < numImages; i++) {
      const filename = `product-${product.id}-biftek-${i + 1}.jpg`;
    ...
    console.log('✅ Biftek viçi images fixed!');
    process.exit(0);
  } catch (error) {
