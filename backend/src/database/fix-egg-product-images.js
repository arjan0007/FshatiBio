...
// URL të vlefshme për foto reale të vezëve nga Unsplash
const eggImageUrls = {
  'Vezë të freskëta': [
    'https://images.unsplash.com/photo-1582722872445-44dc5f7e3c8f?w=800&h=600&fit=crop',
    'https://images.unsplash.com/photo-1518568814500-bf0f82d0c4ce?w=800&h=600&fit=crop',
    'https://images.unsplash.com/photo-1582724658326-05b143bc3c11?w=800&h=600&fit=crop'
  ],
  'Vezë të mëdha': [
    'https://images.unsplash.com/photo-1518568814500-bf0f82d0c4ce?w=800&h=600&fit=crop',
    'https://images.unsplash.com/photo-1582722872445-44dc5f7e3c8f?w=800&h=600&fit=crop',
...
async function fixEggProductImages() {
  try {
    console.log('🥚 Fixing egg product images...');
    
    const products = await pool.query(`
      SELECT p.id, p.name, p.image_urls
      FROM products p
      WHERE p.name LIKE '%Vezë%'
    ...
    console.log(`\n📦 Found ${products.rows.length} egg products\n`);
    
    for (const product of products.rows) {
      try {
        const imageUrls = eggImageUrls[product.name] || [];
        ...
        const allUrls = [...imageUrls, ...alternativeEggUrls];
    ...
    console.log('✅ Egg product images fixed!');
    process.exit(0);
  } catch (error) {
